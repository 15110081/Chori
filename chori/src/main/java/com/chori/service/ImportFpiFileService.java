...
@Repository("importFpiFileService")
public class ImportFpiFileService {
	 ...
	 */
	public boolean readFpiGridFromExcelFile(String excelFilePath, String lotNumber) throws IOException {
	    ...
	    while (iterator.hasNext()) {
	        ...
	        while (cellIterator.hasNext()) {
	            ...
	            if(getCellValue(nextCell)!=(null)){
	            	if(((String) getCellValue(nextCell).toString()).equals("A4-2-2")){
		            	System.err.println("row: "+nextCell.getRowIndex()+" column: "+org.apache.poi.ss.util.CellReference.convertNumToColString(nextCell.getColumnIndex()));
		            	garmentColumnIndex = nextCell.getColumnIndex();
		            }
	            }
	            
	            columnIndex = nextCell.getColumnIndex();
	            
	            if(nextCell.getRowIndex()>13){
	            	
	            	switch (columnIndex) {
		            	case 0:
		            		if(getCellValue(nextCell)!=(null)){
		            			DataFormatter formatter = new DataFormatter();
			            		sizeValue = formatter.formatCellValue(nextCell);
//			            		System.err.println("Size: " + sizeValue);
		            		}else{
		            			sizeValue= null;
		            		}
		            		break;
		            	case 1:
		            		if(getCellValue(nextCell)!=(null)){
		            			DataFormatter formatter = new DataFormatter();
			            		colorValue = formatter.formatCellValue(nextCell);
//			            		System.err.println("Color: " + colorValue);
	            	...
	            	//nếu cột đang đọc chạy tới index của cột garment
	            	if(columnIndex==garmentColumnIndex){
	            		if(getCellValue(nextCell)!=(null)){
	            			DataFormatter formatter = new DataFormatter();
		            		fpiValue = (int) ((Double) getCellValue(nextCell)).floatValue();
//		            		System.err.println("Fpi value: " + fpiValue);
//		            		System.err.println();
	            		}else{
	            			fpiValue = -1;
	            		}
	            	...
	            	if(sizeValue!= null&&colorValue!= null&&fpiValue!=-1){
	            		System.err.println(sizeValue+ " " + colorValue + " " + fpiValue);
	            	}
	            }
