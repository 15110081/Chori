$(document).ready(function(){
	...
	function loadDataForFabricAssignment(){
		$("#listFabricAssignment").dataTable().fnDestroy();
		$('#listFabricAssignment tbody').empty();
		$('#fabricAssignmentListDialog').find('#txtLotNumber').val($('#editPIdialog').find("#lotnumber").val());
		 $('#fabricAssignmentListDialog').find('#txtFactoryName').val($('#editPIdialog').find("#factorycode").val());
		$('#fabricAssignmentListDialog').find('#txtCustomerName').val($('#editPIdialog').find("#customer1").val());
		var factoryCode = $('#fabricAssignmentListDialog').find('#txtFactoryName').val();
		var customerCode = $('#fabricAssignmentListDialog').find('#txtCustomerName').val();
		var lotNumber = $('#fabricAssignmentListDialog').find('#txtLotNumber').val();
...
//		var lotNumber = $("#txtLotNumber").val();
		...
		$.ajax({
			...
			url: "/Chori/fabricassignment/list",
...
//					{
	...
	$('#btnSubmit').on('click', function (e) {
		...
		var fabricnoArrayIsCheckedUnique=[]; 
		var fabricnoArrayIsNotCheckedUnique =[];
		...
		fabricnoArrayIsCheckedUnique = fabricnoArrayIsChecked.reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[]);
		fabricnoArrayIsNotCheckedUnique = fabricnoArrayIsNotChecked.reduce(function(a,b){if(a.indexOf(b)<0)a.push(b);return a;},[]);
		...
		for(var i =0;i<fabricnoArrayIsCheckedUnique.length;i++)
		{
			fabricno = $("#chk"+fabricnoArrayIsCheckedUnique[i]+"").data("id");
			listAssign.push({
				lotnumber: lotnumber,
				fabricno: fabricno
			});
		...
		var fabricno;
		var listAssign1 = [];
		for(var i =0;i<fabricnoArrayIsNotCheckedUnique.length;i++)
		{
			fabricno = $("#chk"+fabricnoArrayIsNotCheckedUnique[i]+"").data("id");
			listAssign1.push({
				lotnumber: lotnumber,
				fabricno: fabricno
			});
		...
		$.ajax({
			...
			async:false,
			data:
				JSON.stringify(listAssign1),
			contentType: "application/json",
			url: "/Chori/piassignfabric/delete/",
			success: function(data){
	...
	function reloadPiGrid(lotnumber){
		/**
		 * Test
		 */
		var lotnumber = $("#fabricAssignmentListDialog").find("#txtLotNumber").val();
	...
	function action(){
		...
		$('.btnFabricAssignment').on('click', function (e){
			...
			$.ajax({
				...
				url: "/Chori/piassignfabricdetail/detail",
				contentType: "application/json",
				success: function(data){
	            	...
	            	$.each(data.piassignfabricdetail, function( key, value ) {  
						...
						var garmentStyleString = garmentStyle.split("@@@",1);
						var color = (value.color==null?"":value.color);
						$('<tr>').append(
							$('<td>').text(value.lotnumber),
							$('<td>').text(garmentStyleString),	
							$('<td>').text(color),	
		...
		var assignQtyPercent = [] ;
		 ...
		 $("#listAddNewFabricAssignmentCover").html("");
			...
			$.ajax({
				...
				url: "/Chori/listfabricassignmentquantity",
				contentType: "application/json",
				success: function(data){
						var i=0					
						$.each(data.list,function(key,value){					
    						...
    						$("input[id='txtAssignQuantityPercent" + value.color+garmentStyleString+i+"']").on('input propertychange paste', function (e) {	
    							var calQtyByPercent = parseInt($("#listAddNewFabricAssignment").find("input[id='txtAssignQuantityPercent" + value.color+garmentStyleString+i+"']").val() * (value.sumAssignQtyAndAvailableQtyToCalPercent /100));
    							$("#listAddNewFabricAssignment").find("label[id='txtAssignQty" + value.color+garmentStyleString+i+"']").text(calQtyByPercent);
    						});
    						
    						
//    						("input[id='txtAccessoryGroupCode" + value.accessorygroupcode + "']")
//    						.find("input[id='txtAccessoryGroupCode" + value.accessorygroupcode + "']").val()
    						
    						
    						$("#btn100PercentAssign").on('click', function (e) {  		
    							var max = parseFloat($("#listAddNewFabricAssignment")
    									.find("input[id='txtAssignQuantityPercent" + value.color+garmentStyleString+i+"']").prop('max')).toFixed(2);
    							var qtyPercent = parseFloat($("#listAddNewFabricAssignment")
    									.find("input[id='txtAssignQuantityPercent" + value.color+garmentStyleString+i+"']").val(max));
    							var calQtyByPercent =  parseInt($("#listAddNewFabricAssignment")
    									.find("input[id='txtAssignQuantityPercent" + value.color+garmentStyleString+i+"']").val() * value.sumAssignQtyAndAvailableQtyToCalPercent /100 );
    							$("#listAddNewFabricAssignment").find("label[id='txtAssignQty" + value.color+garmentStyleString+i+"']").text(calQtyByPercent);
    						});
    						
    						$("#btnReset100Percent").on('click', function (e) {  
    							var qtyPercent = $("#listAddNewFabricAssignment").find("input[id='txtAssignQuantityPercent" + value.color+garmentStyleString+i+"']").val(0);
    							qtyPercent = 0;
		...
		$("#addNewFabricAssignmentDialog").dialog({
			...
			buttons:{			
				"Save": function(){
					...
					{
						lotNo = $('#addNewFabricAssignmentDialog').find('#txtLotNo').val();
						fabricNo =  $('#addNewFabricAssignmentDialog').find('#txtFabricNo').val();
						color = $("input[id='txtAssignQuantityPercent"+assignQtyPercent[i]+ "']").data('color');
						garmentStyle = $("input[id='txtAssignQuantityPercent"+assignQtyPercent[i]+ "']").data('garmentstyle');
						assignqty = $("label[id='txtAssignQty"+assignQtyPercent[i]+ "']").text();
						assignqtypercent = $("input[id='txtAssignQuantityPercent"+assignQtyPercent[i]+ "']").val();
						
						assignquantityarray.push ({
							lotnumber : lotNo,
							fabricno : fabricNo,
						...
						console.log(JSON.stringify(assignquantityarray));	
					}
					...
					$.ajax({
						...
						type: 'POST',
						async: false,					
						data: JSON.stringify(assignquantityarray),
						contentType: "application/json",
						url: getAbsolutePath() + "piassignfabricdetail/save",
						success: function(data){										
						...
						callMessageDialog("Message", 'Save successfully!');
						loadDataForFabricAssignment();
						$("#addNewFabricAssignmentDialog").dialog("close");
